...
(function () {
  ...
  document.addEventListener("click", (e) => {
    if (e.target.id === "btn-export") exportBackup();
  ...
  async function exportBackup() {
    const name = `mission-control-${today()}.json`;
    const text = JSON.stringify(state, null, 2);
    const btn = document.getElementById("btn-export");
    const say = (m) => { if (btn) { btn.textContent = m; setTimeout(() => (btn.textContent = "Export backup"), 2200); } };

    /* Published as an Artifact the page can't hand over a file on its own —
       the viewer has to be asked. Standalone, the link works fine. */
    if (window.claude && typeof window.claude.use === "function") {
      try {
        const dl = await window.claude.use("downloads");
        if (dl) {
          try { await dl.save({ filename: name, data: text }); say("Saved"); }
          catch (err) { say(err && err.code === "declined" ? "Cancelled" : "Couldn't save"); }
          return;
        }
